...
impl Auth for AuthService {
    async fn sign_in(
        ...
        let auth_lock = self
            .users_service
            .lock()
            .expect("auth_lock should not be poisoned");

        let result: Option<String> = auth_lock.get_user_uuid(req.username, req.password); // Get user's uuid from `users_service`. Panic if the lock is poisoned.
        ...
        match result {
            Some(uuid) => {
                ...
                let session_token = session_lock.create_session(&user_uuid);
    ...
    async fn sign_up(
        ...
        let result: Result<(), String> = self
            .users_service
            .lock()
            .expect("User service should not panic")
    ...
    async fn sign_out(
        ...
        let mut sessions_lock = self
            .sessions_service
            .lock()
            .expect("Sessions service should not be posion");

        sessions_lock.delete_session(&req.session_token);
